...
verification and demo assets**, not implementation — see `TODO.md`.
...
| K9 | Call Center UI | **Done** — `frontend/app/calls/[spec_id]/`, `frontend/components/calls/` (DealerCard, CallStatusPanel, TranscriptStream, AudioPlayer, QuoteChip, StateBadge, CharacterCard, RoleplaySession), `frontend/lib/{useCallCenter,characterCards}.ts`. Bridge-mode wired live (start, status poll, transcript, quote, recording); round cap at 2 (round 1 + leverage round 2), round-2 mock content per persona. Role-play promoted from placeholder to **primary** demo call mechanism (bridge overlap since addressed by K5's turn-taking gate; live re-check pending) — 4 character cards sourced from `vertical.json` ratios (no invented PKR figures) + `RoleplaySession.tsx` wrapping `@elevenlabs/react` live; mock mode shows a placeholder since the existing mock timer path already auto-completes. "View report" button gated on all-dealers-terminal. **Mock-verified end-to-end in browser** by the user (call all → round-2 leverage → report → citation round-trip → 375px → print). Real-mode untested — blocked on CORS (C) |
