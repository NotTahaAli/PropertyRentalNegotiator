...
| K10 | Report generator + UI | **UI done (A), gen pending (C)** — `frontend/app/report/[spec_id]/`, `frontend/components/report/` (RankedTable, FlagChip, RecommendationBlock, CitationLink, PriceCell). Ranked by `total_first_year`, flags with reasons (reuses `Quote.flagged`/`flag_reason`, no parallel lookup), recommended deal with plain-language why, `[call N, line M]` citations that round-trip into K9's `TranscriptStream` (scroll+highlight), recording links, print-friendly layout. `Report` type has no separate citation field — `RecommendationBlock` reads `call_number`/`citation_line` off the recommended row itself so table and recommendation can't cite different lines. **Mock-verified end-to-end in browser.** `GET /report/{spec_id}` doesn't exist on the backend yet — real mode surfaces a clean "not available" error until C ships it |
