...
`property_ref` (nullable) tags which of a dealer's several matching properties a quote is for — `null`/`""` both mean "the dealer's one property." A dealer can log more than one quote per spec, one per `property_ref`; `log_quote`'s upsert key is `(call_id, property_ref)`.
...
- `log_quote` — writes itemised quote row mid-call; validates against fee taxonomy; optional `property_ref` discriminates a dealer's several matching properties.
- `get_leverage` — returns best *real* logged bids for the spec, each tagged with its `property`; empty if none exist.
