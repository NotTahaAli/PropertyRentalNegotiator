...
| K9 | Call Center UI | **Mostly done** — `frontend/app/calls/[spec_id]/`, `frontend/components/calls/` (DealerCard, CallStatusPanel, TranscriptStream, AudioPlayer, QuoteChip, StateBadge), `frontend/lib/useCallCenter.ts`. Bridge-mode calls wired live (start, status poll, transcript, quote, recording); mock mode; persona dropdown; flagged badge; a completed call's "Call again" starts round 2 (leverage round) automatically. **Gap:** role-play "Answer as dealer" toggle renders a placeholder — the ElevenLabs widget embed (negotiator agent + dynamic vars, same pattern as K8 VoiceIntake) is not wired, so the K5-fallback demo path has no UI (`TODO.md`). Not live-verified end-to-end |
